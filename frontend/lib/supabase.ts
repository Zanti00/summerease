 ...
 */
export async function uploadToSupabaseBucket(file: File, bucket = "documents"): Promise<string> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.message || `Supabase storage upload failed with status ${response.status}: ${response.statusText}`
    );
  }
 ...
 */
export async function listFilesInBucket(bucket = "documents"): Promise<SupabaseFile[]> {
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const supabaseKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}));
    throw new Error(
      errorData.message || `Supabase storage list failed with status ${response.status}: ${response.statusText}`
    );
  }
 ...
 */
export async function deleteFileFromBucket(filePath: string, bucket = "documents"): Promise<void> {
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const supabaseKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;
