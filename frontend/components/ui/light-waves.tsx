...
export function LightWavesBackground({
  ...
  // start with undefined so we can detect whether a frame is scheduled
  const animationRef = useRef<number | undefined>(undefined);
  // avoid calling Date.now() during render; set later in effect
  ...
  useEffect(() => {
    ...
    // initialize timestamp when we have a rendering context
    ...
    const updateSize = () => {
      ...
      // update canvas width anytime it changes (scrollbar toggles etc.) but
      // only reinitialize waves when the height actually changes.  The wave
      // geometry is based on height, so a width-only change shouldn't restart
      // the animation.
      if (rect.width !== width) {
        width = rect.width;
        canvas.width = width;
  ...
  return (
    <>
      ...
      {/* Place children outside of the fixed container so they flow naturally and can scroll */}
      {children && <div className="relative z-10 w-full">{children}</div>}
    </>
  );
}

// Default export is the background component itself so you can pass props & children
export default LightWavesBackground;

// Optional demo helper if you still want to render without props
export function LightWavesBackgroundDemo() {
