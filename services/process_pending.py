...
async def enqueue_pending():
    ...
    async with async_session_maker() as db:
        res = await db.execute(text("SELECT id, title, storage_path, processing_status FROM documents WHERE deleted_at IS NULL AND (processing_status = 'pending' OR processing_status = 'failed')"))
        docs = res.fetchall()
        if not docs:
            print("No pending/failed documents found.")
            return
        ...
        print("Successfully enqueued all pending/failed documents!")
